...
# that the signature is redacted, and that the raw signature segment never leaks.
set -euo pipefail
...
fail=0
for needle in '"kid": "test-key-id"' '"iss": "https://token.actions.githubusercontent.com"' '"environment": "production"' '"aud": "cloudsmith"' '── header ──' '── payload ──' '── signature ──' '<redacted>'; do
  if ! grep -qF "$needle" <<<"$out"; then
    echo "MISSING: $needle"; fail=1
  fi
done

# The signature segment must never appear in the output — printing it would let
# someone reassemble a complete, verifiable token.
if grep -qF "$signature" <<<"$out"; then
  echo "LEAKED SIGNATURE SEGMENT: $signature"; fail=1
fi
